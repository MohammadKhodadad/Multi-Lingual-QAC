...
JRC runs also update `reports/jrc-acquis/jrc_pipeline_generation_report.md` after raw loading, corpus building, QA source selection, QA generation, and HF upload. The report consolidates every available numerical audit count: raw documents, chemical filtering, cleaning loss, multilingual CELEX coverage, QA-candidate rejection reasons, selected generation candidates, translation failures, cross-language support failures, final QAC rows, and artifact row counts.
