...
- Added `--jrc-qa-filter-profile` for JRC corpus building: `1`/`strict` preserves the original QA-candidate minimums, while `2`/`soft` lowers the minimum length and paragraph thresholds to admit more compact legal passages. If omitted in interactive mode, JRC corpus building asks which profile to use.
...
- Metrics now include `Recall`, `MAP`, `nDCG`, and same-language irrelevant-result share at `k=10,20,50,100`, plus `same_language_irrelevant_share_at_100_lang_{lang}` diagnostics for `de`, `en`, `es`, `fr`, `pt`, and `zh`; generated JSON/CSV artifacts keep the full metric set, while Markdown/LaTeX comparison tables stay compact with the headline 10/100 columns.
