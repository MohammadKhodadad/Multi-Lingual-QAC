...
\subsection{Construction Audit}

Each JRC construction run writes a numerical audit report to \texttt{reports/jrc-acquis/jrc\_pipeline\_generation\_report.md}. This report is not an evaluation result, but it is part of the experimental record because it fixes the dataset instance being evaluated. It records the number of raw documents loaded, documents retained after optional chemical filtering, cleaned retrieval documents, multilingual \texttt{CELEX} groups, QA candidates, sampled generation candidates, accepted and skipped source rows, translation failures, cross-language support failures, final QAC rows, and artifact row counts. The final paper should cite the audit counts from the frozen dataset run before reporting model scores.

\subsection{Results To Be Finalized}
...
\begin{table}[t]
\centering
\caption{Main multilingual retrieval results to be filled from the finalized benchmark run.}
...
\subsection{Analysis To Be Finalized}
